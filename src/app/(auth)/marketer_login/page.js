...
import { AlertCircle, ArrowRight, Eye, EyeOff, Mail, Users, Send, FileText, TrendingUp } from 'lucide-react'
...
const METRICS = [
  { icon: Users,      label: 'Active Subscribers', value: 2840, suffix: '',  color: 'var(--secondary)' },
  { icon: Send,       label: 'Campaigns Sent',     value: 63,   suffix: '',  color: 'var(--accent)'    },
  { icon: FileText,   label: 'Email Templates',    value: 18,   suffix: '',  color: 'var(--secondary)' },
  { icon: TrendingUp, label: 'Delivery Rate',      value: 98,   suffix: '%', color: 'var(--accent)'    },
]
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res  = await fetch('/api/auth/login', {
        ...
        body: JSON.stringify({ email, password, portal: 'marketer' }),
      })
      ...
      <div className="hidden lg:flex flex-col relative overflow-hidden bg-surface border-r border-border" style={{ width: '42%' }}>
        ...
        <div className="relative z-10 px-8 pt-8">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg mb-5 bg-secondary/8 border border-secondary/18">
            ...
            <span className="text-[9px] font-black tracking-[0.25em] uppercase text-secondary">Marketing Panel</span>
          </div>
          <h2 className="font-black leading-[0.9] tracking-tight text-foreground" style={{ fontSize: 'clamp(2rem, 3.5vw, 2.8rem)' }}>
            GoFast<br />
            <span className="text-secondary">Email</span><br />
            Marketing
          </h2>
          <p className="mt-4 text-xs leading-relaxed text-muted max-w-[240px]">
            Manage subscribers, build templates, and send campaigns.
          </p>
        </div>

        <div className="relative z-10 px-8 pt-8 flex flex-col gap-2.5">
          <p className="text-[9px] font-black tracking-[0.25em] uppercase mb-1 text-muted font-mono">▸ Live Marketing Stats</p>
        ...
        <div className="relative z-10 mt-auto px-8 pb-8 pt-6">
          ...
          <p className="text-[10px] text-muted font-mono">
            Go Fast Delivery Inc. Marketing Panel<br />Authorized access only
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center px-6 sm:px-12 lg:px-12 py-12 relative">
        ...
        <div className="w-full max-w-sm mx-auto">
          ...
          <div className="mb-8" style={{
            ...
            <p className="text-sm mt-1.5 text-muted">Restricted · Email marketers only</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4" style={{
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                placeholder="marketer@gofastdelivery.ca"
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? (
                ...
                <><Mail size={14} strokeWidth={2.5} />Access Dashboard<ArrowRight size={14} className="group-hover:translate-x-0.5 transition-transform" /></>
              )}
            </button>
          </form>
